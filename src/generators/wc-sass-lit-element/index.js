...
export const WcLitElementSassMixin = subclass =>
  class extends subclass {
    async execute() {
      ...
      // copy sass styles
      this.copyTemplate(
        `${__dirname}/templates/_MyEl.scss`,
        this.destinationPath(`src/${className}.scss`),
      );
...
export const WcLitElementSassPackageMixin = subclass =>
  class extends CommonRepoMixin(WcLitElementSassMixin(subclass)) {
    async execute() {
      await super.execute();
      ...
      this.copyTemplate(
        `${__dirname}/templates/webpack.config.js`,
